...
1. **SUUMO で物件検索** — 実行ランタイムに応じてブラウザ操作
   - **Claude Code** の場合: Claude in Chrome (`mcp__Claude_in_Chrome__*` ツール群) で SUUMO ページを開いて調査する
   - **Codex CLI** の場合: Playwright CLI を使ってスクレイピング
...
- 実行ランタイム別の前提:
  - **Claude Code**: Claude in Chrome MCP (`mcp__Claude_in_Chrome__*`) が利用可能であること
  - **Codex CLI**: Node.js, direnv と Playwright CLI がインストールされていること
...
- 実行ランタイムに応じてブラウザ操作ツールを切り替える
  - **Claude Code**: Claude in Chrome (`mcp__Claude_in_Chrome__*`) で SUUMO ページを開いて調査する
  - **Codex CLI**: Playwright CLI を使う
- いずれの場合も、毎回フォームを手操作するのではなく、まず駅ページから検索 URL を固定化する
...
### 最短手順 (Claude Code / Claude in Chrome)

1. ブラウザの起動確認
   - `mcp__Claude_in_Chrome__navigate` で対象 URL を開ける状態になっているか確認する
   - 未起動なら新しいタブを作成 (`mcp__Claude_in_Chrome__tabs_create_mcp`) する

2. 対象駅ページを開く
   - `mcp__Claude_in_Chrome__navigate` で駅ごとの SUUMO ページに遷移する
   - 例: 中野駅は `https://suumo.jp/chintai/tokyo/ek_27280/`
   - 以前誤って別駅コードを踏んだため、`mcp__Claude_in_Chrome__read_page` などでページタイトルから駅名を必ず確認する

3. 高度条件は UI 手入力より DOM / URL を優先する
   - `さらに条件を追加する` を開く必要があるときは `mcp__Claude_in_Chrome__javascript_tool` で DOM を直接調べる
   - フォーム内の field 名と value を確認する
   - 例:
     - 家賃上限: `ct=23.0`
     - 管理費込み: `co=1`
     - 1LDK: `md=04`
...
5. 一覧から候補を取る
   - `mcp__Claude_in_Chrome__get_page_text` または `javascript_tool` で `div.cassetteitem` ごとに以下を取得する
     - 物件名
     - 住所
     - アクセス
     - 築年数
...
6. 向きは詳細ページで確認する
   ...
   - 各候補の詳細 URL を `navigate` で開き、`get_page_text` などで `向き` を確認し、最終候補だけ残す
   - 実測では北向き・西向き物件が一覧に残るため、この確認は必須

### 最短手順 (Codex CLI / Playwright CLI)
...
2. 対象駅ページを開く・条件確認・URL 固定化・一覧取得・向き確認
   - 上記 Claude in Chrome 版の手順 2〜6 と同じロジックを Playwright CLI で行う
   - 操作は `goto` / `eval` / `run-code` 等に置き換える

### 実行メモ

- Claude in Chrome の `mcp__Claude_in_Chrome__javascript_tool` は DOM の状態確認や value の抽出に向いている。`document.querySelector(...)` の結果から field 名や option value を取る
- Playwright CLI 利用時:
  ...
  - `click` / `check` が lightbox overlay に阻害される場合は、DOM から value / checked をセットして検索 URL を確定する方が速い
